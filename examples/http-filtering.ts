...
async function httpExample() {
  ...
  console.log(
    "- Spec Loading: URL (https://petstore.swagger.io/v2/swagger.json)",
  );
  console.log("- Transport: HTTP on http://127.0.0.1:3003/mcp");
  console.log(
    '- Filtering: Only "pet" and "store" tags, only GET/POST methods',
  );
  ...
  try {
    ...
    console.log(
      `- MCP Protocol: POST http://127.0.0.1:3003/mcp (with JSON-RPC)`,
    );
    ...
    try {
      ...
      console.log(
        "⚠️  HTTP endpoint test failed (this is expected if fetch is not available)",
      );
    }
      ...
      JSON.stringify(
        {
          mcpServers: {
            "petstore-api": {
              command: "npx",
              args: ["ts-node", "examples/http-filtering.ts"],
            },
          },
        },
        null,
